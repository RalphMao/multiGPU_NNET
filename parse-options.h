...
class OptionsItf {
 public:
  ...
  virtual void Register(const std::string &name,
                int *ptr, const std::string &doc) = 0; 
  virtual void Register(const std::string &name,
...
class ParseOptions : public OptionsItf {
 public:
  explicit ParseOptions(const char *usage) :
    ...
    prefix_(""), other_parser_(NULL), program_name(NULL) {
    ...
    RegisterStandard("verbose", &verbose_level,
  ...
  ParseOptions(const std::string &prefix, ParseOptions *other) :
    ...
    prefix_(prefix), other_parser_(other), program_name(NULL) {}

  ~ParseOptions() { if( program_name ) free(program_name); }
