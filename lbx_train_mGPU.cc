...
//	}
	...
	if( nn_in_dim[0] != feature_out_dim ) {
		printf("nn_in_dim = %d, feature_out_dim = %d\n", nn_in_dim, feature_out_dim );
		printf("Mismatch!\n");
	}

	int nn_in_stride = ( (nn_in_dim[0] + 31)/ 32 ) * 32;
...
void *lbxParallelTrainLock ( void *arg ) {
	...
	float *nn_para_delta_cpu = (float *)malloc( nn_para_num * sizeof(float) ); 
	...
	while ( !feof( file_pointer_key ) || size_row_filled_in_buffer >0 ) {
		...
		lbxAddRowToMatrixThenRescaleGPU( size_row_filled_in_buffer, nn_in_dim[0], nn_in_stride, 
		...
		while ( size_row_filled_in_buffer >= batch_size ) {
			...
			lbxNetPropagation( nn_para_gpu, nn_hidden_result_gpu, 
				nn_in_gpu + (index_top_of_the_buffer * nn_in_stride), nn_out_gpu,
			...
			if( !cv_flag ) {
				...
				lbxNetBackProp( nn_para_gpu, 
					...
					nn_in_gpu + (index_top_of_the_buffer * nn_in_stride), nn_diff_gpu,
					nn_layer_num, batch_size, 
					nn_in_dim, nn_out_dim, 
					nn_in_stride, nn_out_stride, 
					nn_weight_pointer_stride, nn_bias_pointer_stride, nn_result_pointer_stride, 
	...
	*(thread_arg_t->correct_all_merge_s) += correct_all;
	...
	cudaFree(vec_one_gpu);
	
	free(nn_in_dim);
	free(nn_out_dim);
	
	free( nn_para_delta );
	free( nn_para_delta_cpu );
